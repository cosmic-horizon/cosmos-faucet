...
// Add this at the top of your code to maintain a set of used addresses.
const usedAddresses = new Set();

// Modify the sendTx function to check and mark addresses as used.
function sendTx(addressTo, res) {
	if (usedAddresses.has(addressTo)) {
		res.send({ error: 'Address has already been used.' });
		return;
	}
	...
	cosmos.getAccounts(address).then(data => {
		...
		// After successfully broadcasting the transaction, mark the address as used.
		return cosmos.broadcast(signedTxBytes).then(function (response) {
			usedAddresses.add(addressTo);
			res.send({ response });
...
app.get('/', function (req, res) {
	...
	const addressTo = req.query.address;

	if (addressTo === '') {
		console.log(false);
		res.send({ error: 'Already funded' });
	} else {
		var rtnTx = sendTx(addressTo, res);
...
app.listen(config.lport, function () {
	console.log('***********************************************')
	console.log('* Welcome to Cosmos-faucet')
